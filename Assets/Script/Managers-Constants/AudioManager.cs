...
public class AudioManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        {
            Debug.LogWarning("하늘 아래에 두 개의 태양은 있을 수 없다..(오디오매니저는 2개일 수 없습니다.)");
            Destroy(gameObject);
        }
    ...
    void Start(){
        ...
        Debug.Log(" 이벤트 등록(OnPlayerSpawn += OnPlayerSpawn)"  + gameObject.name);
    }
    ...
    void Update()
    {
        AudioListenerToPlayer();
    }


    void AudioListenerToPlayer()
    {
        if (!playerT)
            return;
    ...
    void OnPlayerSpawn(Transform playerT){
        this.playerT = playerT;
        Debug.Log("플레이어가 생성됐고, 오디오리스너가 이제 따라갑니다.");
    }
